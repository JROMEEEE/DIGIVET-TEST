...
export default function Register() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (data.session) navigate('/vet', { replace: true });
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#fafafa', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
        <div style={{ textAlign: 'center', maxWidth: 420, padding: '2rem' }}>
          ...
          <Link to="/login" style={{ background: MAROON, color: '#fff', padding: '0.7rem 1.5rem', borderRadius: '8px', textDecoration: 'none', fontWeight: 600, fontSize: '0.9rem' }}>
            Back to sign in
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fafafa', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 69px)', padding: '2rem' }}>
        <div style={{ width: '100%', maxWidth: 440, background: '#fff', borderRadius: '16px', padding: '2.5rem', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: '1px solid #f0f0f0' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            ...
            <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#111', margin: '0 0 0.4rem' }}>Veterinarian Registration</h1>
            <p style={{ color: '#777', fontSize: '0.88rem', margin: 0 }}>Create a veterinarian account for the DIGIVET portal</p>
          </div>

          <div style={{ background: '#fffbea', border: '1px solid #f0d080', borderRadius: '8px', padding: '0.7rem 1rem', fontSize: '0.82rem', color: '#7a5800', marginBottom: '1.25rem', lineHeight: 1.5 }}>
            ⚕️ Veterinarian accounts require admin verification before full access is granted.
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div>
              ...
              <input type="text" required value={form.fullName} onChange={e => setForm(f => ({ ...f, fullName: e.target.value }))} placeholder="Dr. Juan dela Cruz" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Email address</label>
              <input type="email" required autoComplete="off" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="you@example.com" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Password</label>
              <input type="password" required autoComplete="new-password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="Minimum 6 characters" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Confirm password</label>
              <input type="password" required autoComplete="new-password" value={form.confirmPassword} onChange={e => setForm(f => ({ ...f, confirmPassword: e.target.value }))} placeholder="Repeat password" style={inputStyle} />
            </div>
            <button
              type="submit" disabled={submitting}
              style={{ width: '100%', background: submitting ? '#b0b0b0' : MAROON, color: '#fff', border: 'none', borderRadius: '8px', padding: '0.8rem', fontWeight: 700, fontSize: '0.95rem', cursor: submitting ? 'not-allowed' : 'pointer', marginTop: '0.25rem' }}
